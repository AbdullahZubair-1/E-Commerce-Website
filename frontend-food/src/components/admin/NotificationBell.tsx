        ...
        <div className="absolute right-0 mt-2 w-80 max-h-96 overflow-y-auto bg-white rounded-2xl shadow-xl border border-gray-200 z-50">
          ...
          {notifications.map((n) => (
            <button
              ...
              className={`w-full text-left flex gap-3 px-4 py-3 border-b border-gray-50 hover:bg-gray-50 transition-colors ${!n.is_read ? 'bg-orange-50/50' : ''}`}
            >
              ...
              {!n.is_read && <span className="h-2 w-2 rounded-full bg-orange-600 mt-1.5 shrink-0" />}
            </button>
          ))}
        </div>
      )}
