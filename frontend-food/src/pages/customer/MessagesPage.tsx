        ...
        <div className="md:col-span-1 bg-white border border-gray-200 rounded-2xl flex flex-col overflow-hidden">
          <div className="p-3 border-b border-gray-100">
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-2 text-sm rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-orange-100 focus:border-orange-400"
              />
            </div>
                ...
                {searchResults.map((r) => (
                  <div key={r.id} className="flex items-center justify-between px-2 py-1.5 rounded-lg hover:bg-gray-50">
                    ...
                    <button
                      ...
                      className="flex items-center gap-1 text-xs text-orange-600 hover:text-orange-700 shrink-0 ml-2"
                    >
          ...
          <div className="flex border-b border-gray-100 text-sm font-medium">
            <button
              ...
              className={`flex-1 py-2.5 ${tab === 'friends' ? 'text-orange-600 border-b-2 border-orange-600' : 'text-gray-500'}`}
            >
              Friends
            </button>
            <button
              ...
              className={`flex-1 py-2.5 relative ${tab === 'requests' ? 'text-orange-600 border-b-2 border-orange-600' : 'text-gray-500'}`}
            >
              Requests
              {incoming.length > 0 && (
              ...
              friends.length === 0 ? (
                ...
                friends.map((f) => (
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-3 py-3 text-left hover:bg-gray-50 transition-colors ${activeFriendId === f.id ? 'bg-orange-50' : ''}`}
                  >
                    <div className="relative shrink-0">
                      <div className="h-9 w-9 rounded-full bg-orange-100 text-orange-700 flex items-center justify-center font-semibold text-sm">
                        {f.first_name[0]}
                        {f.last_name[0]}
                      </div>
                      {f.online && (
            ...
            <>
                    ...
                    <div key={m.id} className={`flex ${mine ? 'justify-end' : 'justify-start'}`}>
                      <div
                        className={`rounded-2xl px-4 py-2 max-w-[75%] text-sm ${
                          mine ? 'bg-orange-600 text-white' : 'bg-white text-gray-900 shadow-sm'
                        }`}
                      >
                        {m.content}
                      </div>
              ...
              <div className="p-3 border-t border-gray-100 flex gap-2">
                <input
                  ...
                  className="flex-1 rounded-xl border border-gray-200 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-orange-100 focus:border-orange-400"
                />
                <button
                  type="button"
                  onClick={handleSend}
                  disabled={!draft.trim()}
                  className="inline-flex items-center justify-center rounded-xl bg-orange-600 px-4 py-2.5 text-white hover:bg-orange-700 disabled:opacity-40 disabled:cursor-not-allowed"
                >
