    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="flex items-start justify-between mb-6 flex-wrap gap-4">
        ...
        <button
          ...
          className="flex items-center gap-2 text-sm text-orange-600 hover:text-orange-800 border border-orange-200 hover:border-orange-400 rounded-lg px-3 py-1.5 transition-colors disabled:opacity-50"
      ...
      <div className={`border rounded-xl p-4 mb-6 flex items-center gap-3 ${getStatusBannerStyle(order.status)}`}>
        {isCancelled ? (
          ...
          <TruckIcon className="h-6 w-6 flex-shrink-0 text-orange-500" aria-hidden="true" />
        )}
        <div>
                  ...
                  <div className="flex flex-col items-center flex-shrink-0">
                    <div
                      className={`h-10 w-10 rounded-full flex items-center justify-center border-2 z-10 transition-all duration-300 ${
                        isCompleted
                          ...
                          : isCurrent
                          ? 'bg-white border-orange-600 shadow-md shadow-orange-100'
                    ...
                    >
                      {isCompleted ? (
                        ...
                        <Icon
                          className={`h-5 w-5 ${
                            isCurrent ? 'text-orange-600' : 'text-gray-300'
                          }`}
                        />
                      )}
                    </div>
          ...
          {order.items.map((item, idx) => (
            ...
            >
              <div className="flex items-center gap-3">
                {/* Index circle */}
                <div className="h-7 w-7 rounded-full bg-orange-50 flex items-center justify-center flex-shrink-0">
      ...
      <div className="mt-6 text-center">
        <Link
          to="/products"
          className="text-sm text-orange-600 hover:text-orange-800 no-underline font-medium"
        >
          Continue Shopping →
        </Link>
      </div>
