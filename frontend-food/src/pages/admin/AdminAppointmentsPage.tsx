    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        {tab === 'doctors' && (
          <button
            ...
            className="inline-flex items-center gap-2 rounded-xl bg-orange-600 px-4 py-2.5 text-sm font-medium text-white hover:bg-orange-700"
          >
      ...
      <div className="flex border-b border-gray-200 mb-6">
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2.5 text-sm font-medium ${tab === 'doctors' ? 'text-orange-600 border-b-2 border-orange-600' : 'text-gray-500'}`}
        >
        ...
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2.5 text-sm font-medium ${tab === 'bookings' ? 'text-orange-600 border-b-2 border-orange-600' : 'text-gray-500'}`}
        >
          ...
          {showForm && (
            <form onSubmit={handleAddDoctor} className="bg-white rounded-2xl border border-gray-200 p-5 mb-6 space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-100 focus:border-orange-400"
                  />
                </div>
                <div>
              ...
              <button
                type="submit"
                disabled={submitting}
                className="rounded-xl bg-orange-600 px-4 py-2 text-sm font-medium text-white hover:bg-orange-700 disabled:opacity-50"
              >
              ...
              <tbody className="divide-y divide-gray-100">
                {appointments.map((a) => (
                  <tr key={a.id}>
                    ...
                    <td className="px-5 py-3">
                      <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${a.status === 'booked' ? 'bg-orange-100 text-orange-700' : 'bg-gray-100 text-gray-500'}`}>
                        {a.status}
                      </span>
                    </td>
                  </tr>
